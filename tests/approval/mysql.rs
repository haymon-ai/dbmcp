...
#[sqlx::test]
async fn test_server_info(pool: MySqlPool) {
    common::run_with_client(MysqlAdapter::from_pool(pool, false), |peer| async move {
...
#[sqlx::test]
async fn test_list_tools(pool: MySqlPool) {
    common::run_with_client(MysqlAdapter::from_pool(pool, false), |peer| async move {
...
#[sqlx::test]
async fn test_list_tools_read_only(pool: MySqlPool) {
    common::run_with_client(MysqlAdapter::from_pool(pool, true), |peer| async move {
        let tools = peer.list_all_tools().await.expect("list_all_tools failed");
