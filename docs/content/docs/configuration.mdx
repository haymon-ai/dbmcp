...
> **Security note**: Read-only mode is **enabled by default**. In this mode, only `SELECT`, `SHOW`, `DESC`, `DESCRIBE`, and `USE` statements are allowed. Set `--read-only=false` or `MCP_READ_ONLY=false` to allow write operations.
