...
Database MCP exposes six tools through the MCP protocol. AI assistants call these tools to explore and query your database.

### `list_databases`

Discover all accessible databases on the connected server. This is typically the first tool an assistant calls to understand what data is available.

### `list_tables`

List all tables in a specific database. Requires a database name from `list_databases`.
...
Run a SQL query against a specific database. In read-only mode (the default), only `SELECT`, `SHOW`, `DESCRIBE`, and `USE` statements are allowed. See [Security](/docs/security) for details on query validation.

### `create_database`

Create a new database. This tool is blocked when read-only mode is enabled (the default). Not supported for SQLite.
